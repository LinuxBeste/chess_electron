...
export default function TournamentsTab() {
  ...
  return (
    <div>
      <div className="mb-4">
        <SearchBar value={query} onChange={setQuery} placeholder="Search tournaments by name or ID..." />
      </div>

      {filtered.length === 0 ? (
        <p className="text-[#666]">{query ? 'No matching tournaments.' : 'No tournaments.'}</p>
      ) : (
